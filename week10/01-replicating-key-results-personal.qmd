...
does_not_fit <- function(words, embedding_matrix = embeddings) {
  ...
  # this gives cosine similrity because vectors were normalised before computing the dot product
...
analogies <- function(a, b, c, top_n = 1, embedding_matrix = embeddings) {
  ...
  # we remove the original vectors a,b,c because the space is so dense they might reappear again so we have to manuallt remove them to get good results 
