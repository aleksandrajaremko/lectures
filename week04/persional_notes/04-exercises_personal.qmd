...
sounds <- c(
  ...
  "law" = "\\b(?:law(?:ful|yer|yers|s|fully)|constitution(?:al|s)?\\b", 
  "freedom" = "\\b(?:free(?:dom|ing|ly|doms|ed)?|liber(?:ty|ties|ate|ated|ation|ating)\\b"
)
...
free <- tibble(address = row.names(free), freedom = as.numeric(free)) %>%
  ...
  filter(row_number() <= 4 | row_number() > (nrow(.) - 4))

print(free)

#### HOW DO YOU ASCEND IT IN DESCENDING ORDER?
